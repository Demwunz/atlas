...
/// Topo — fast codebase indexer and file selector for LLMs.
#[derive(Parser, Debug)]
#[command(name = "topo", version, about)]
pub struct Cli {
...
mod tests {
    ...
    fn cli_parses_no_args() {
        let cli = Cli::try_parse_from(["topo"]);
        assert!(cli.is_ok());
    }

    #[test]
    fn cli_parses_verbose() {
        let cli = Cli::try_parse_from(["topo", "-v"]).unwrap();
    ...
    fn cli_parses_index_deep() {
        let cli = Cli::try_parse_from(["topo", "index", "--deep"]).unwrap();
        assert!(matches!(
            cli.command,
            Some(Command::Index {
                deep: true,
