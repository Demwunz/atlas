//! Atlas core domain types, traits, and errors.
...
mod tests {
    ...
    #[test]
    fn atlas_error_display() {
        let err = AtlasError::Io("test".to_string());
        assert!(err.to_string().contains("test"));
    }

    #[test]
    fn atlas_error_from_io() {
        let io_err = std::io::Error::new(std::io::ErrorKind::NotFound, "not found");
        let atlas_err: AtlasError = io_err.into();
        assert!(atlas_err.to_string().contains("not found"));
    }
